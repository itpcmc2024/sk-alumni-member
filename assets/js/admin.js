...
function openPrintApplication(m){
 const w=window.open('','_blank','width=900,height=1100');if(!w){uiAlert('ไม่สามารถเปิดหน้าพิมพ์','กรุณาอนุญาต Pop-up สำหรับเว็บไซต์นี้','warning');return;}const logo=new URL('assets/img/association-logo.jpg',location.href).href,addr=[m.houseNo||'',m.moo?'หมู่ '+m.moo:'',m.soi?'ซอย '+m.soi:'',m.road?'ถนน '+m.road:'',m.subdistrict?'ตำบล/แขวง '+m.subdistrict:'',m.district?'อำเภอ/เขต '+m.district:'',m.province?'จังหวัด '+m.province:'',m.postalCode||''].filter(Boolean).join(' ');
 w.document.write(`<!doctype html><html lang="th"><head><meta charset="utf-8"><title>ใบสมัคร ${escapeHtml(m.memberCode)}</title><style>@page{size:A4;margin:14mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#143b30;margin:0}.page{min-height:267mm;position:relative;padding-bottom:18mm}.head{text-align:center}.head img{width:82px;height:82px;object-fit:contain}.head h1{font-size:24px;margin:9px 0 4px}.head h2{font-size:17px;margin:0 0 14px}.greenline{height:2px;background:#16805a;margin:15px 0 16px}.topgrid{display:grid;grid-template-columns:118px 1fr;gap:18px}.photo{width:118px;height:145px;object-fit:cover;border:1px solid #cad8d1}.photo-ph{width:118px;height:145px;border:1px solid #cad8d1;display:grid;place-items:center;color:#82928a}table{width:100%;border-collapse:collapse;font-size:14px}td{border:1px solid #c7d6cf;padding:8px 10px;vertical-align:top}td.label{width:27%;font-weight:700;background:#f7faf8}.section{color:#08794f;font-size:16px;font-weight:800;margin:16px 0 7px}.sign-row{display:grid;grid-template-columns:1fr 1fr;gap:95px;margin-top:62px;text-align:center}.sign-president{width:34%;margin:55px auto 0;text-align:center}.signline{border-top:1px solid #333;padding-top:5px}.foot{position:absolute;right:0;bottom:0;font-size:9px;color:#5f6e67}.actions{position:fixed;right:15px;top:15px}@media print{.actions{display:none}}</style></head><body><button class="actions" onclick="window.print()">🖨 พิมพ์ / Save as PDF</button><div class="page"><div class="head"><img src="${logo}"><h1>ใบสมัครสมาชิก</h1><h2>สมาคมศิษย์เก่านูรุ้ลอิสลามสัมพันธ์ (สุเหร่าเขียว)</h2></div><div class="greenline"></div><div class="topgrid">${m.photoUrl?`<img class="photo" src="${escapeHtml(m.photoUrl)}">`:'<div class="photo-ph">รูปถ่าย</div>'}<table><tr><td class="label">รหัสสมาชิก</td><td><b>${escapeHtml(m.memberCode)}</b></td></tr><tr><td class="label">สถานะ</td><td>${escapeHtml(m.status)}</td></tr><tr><td class="label">วันที่สมัคร</td><td>${escapeHtml(formatDate(m.registeredAt))}</td></tr></table></div><div class="section">ข้อมูลส่วนตัว</div><table><tr><td class="label">ชื่อ-สกุล</td><td>${escapeHtml((m.prefix||'')+' '+(m.fullName||''))}</td></tr><tr><td class="label">ชื่ออาหรับ</td><td>${escapeHtml(m.arabicName||'-')}</td></tr><tr><td class="label">เบอร์โทรศัพท์</td><td>${escapeHtml(formatThaiPhone(m.phone||'-'))}</td></tr><tr><td class="label">อีเมล</td><td>${escapeHtml(m.email||'-')}</td></tr></table><div class="section">ที่อยู่</div><table><tr><td class="label">ที่อยู่</td><td>${escapeHtml(addr||'-')}</td></tr></table><div class="sign-row"><div class="signline">ลายมือชื่อผู้สมัคร</div><div class="signline">เจ้าหน้าที่ผู้ตรวจสอบ</div></div><div class="sign-president"><div class="signline">ประธานสมาคมฯ</div></div><div class="foot">© 2026 SK Alumni Member System by KimhanIkals | V1.0.20</div></div></body></html>`);w.document.close();
}
...
function renderMediaLibrary(){const host=$('#mediaAdminList'),pager=$('#mediaPager');if(!host)return;const q=String($('#mediaSearch')?.value||'').toLowerCase(),folder=$('#mediaFolderFilter')?.value||'',sort=$('#mediaSort')?.value||'newest';let rows=mediaRows.filter(x=>(!folder||x.folder===folder)&&(!q||String(x.name||'').toLowerCase().includes(q)));rows=rows.sort((a,b)=>(sort==='oldest'?1:-1)*(new Date(a.updated)-new Date(b.updated)));const pages=Math.max(1,Math.ceil(rows.length/mediaPageSize));mediaPage=Math.min(mediaPage,pages);const pageRows=rows.slice((mediaPage-1)*mediaPageSize,mediaPage*mediaPageSize);host.innerHTML=pageRows.length?pageRows.map(f=>`<tr><td><button class="media-table-thumb" type="button" onclick="openMediaPreview('${escapeHtml(f.id)}')">${f.preview?`<img src="${escapeHtml(f.preview)}" loading="lazy">`:'📄'}</button></td><td><b class="media-name" title="${escapeHtml(f.name)}">${escapeHtml(f.name)}</b></td><td><span class="media-folder-badge">${escapeHtml(f.folder)}</span></td><td>${(Number(f.size||0)/1024).toFixed(1)} KB</td><td>${escapeHtml(formatDate(f.updated))}</td><td><div class="media-row-actions"><button class="btn btn-view" onclick="openMediaPreview('${escapeHtml(f.id)}')">ดู</button><a class="btn btn-soft" href="${escapeHtml(f.url)}" target="_blank">Drive</a></div></td></tr>`).join(''):'<tr><td colspan="6" class="empty">ไม่พบไฟล์</td></tr>';if(pager)pager.innerHTML=`<span>ทั้งหมด ${rows.length} ไฟล์</span><div><button class="pager-btn" ${mediaPage<=1?'disabled':''} onclick="changeMediaPage(-1)">‹</button><b>หน้า ${mediaPage} / ${pages}</b><button class="pager-btn" ${mediaPage>=pages?'disabled':''} onclick="changeMediaPage(1)">›</button></div>`;}
function changeMediaPage(step){mediaPage+=step;renderMediaLibrary();}
function openMediaPreview(id){const f=mediaRows.find(x=>String(x.id)===String(id));if(!f)return;$('#mediaPreviewTitle').textContent=f.name||'ไฟล์ภาพ';$('#mediaPreviewBody').innerHTML=`${f.preview?`<img class="media-preview-large" src="${escapeHtml(f.preview)}" alt="">`:'<div class="media-file-icon large">📄</div>'}<div class="media-preview-meta"><div><small>หมวด</small><b>${escapeHtml(f.folder||'-')}</b></div><div><small>ขนาด</small><b>${(Number(f.size||0)/1024).toFixed(1)} KB</b></div><div><small>อัปเดต</small><b>${escapeHtml(formatDate(f.updated))}</b></div></div><div class="member-detail-actions"><a class="btn btn-primary" href="${escapeHtml(f.url)}" target="_blank">เปิดใน Google Drive</a>${f.preview?`<button class="btn btn-soft" onclick="copyMediaUrl('${escapeHtml(f.preview)}')">คัดลอก URL รูป</button>`:''}</div>`;$('#mediaPreviewModal').classList.remove('hidden');}
...
let benefitAdminRows=[];
async function loadBenefitsAdmin(){try{const o=await api('adminBenefitsList',{token:adminToken});benefitAdminRows=o.benefits||[];const host=$('#benefitList'),sel=$('#benefitUsageSelect');if(host)host.innerHTML=benefitAdminRows.map(x=>`<div class="benefit-row"><div><b>${escapeHtml(x.title)}</b><small>${escapeHtml(x.description||'')} • มูลค่า ${Number(x.defaultAmount||0).toLocaleString('th-TH')} บาท</small></div><div class="benefit-row-actions"><button class="btn btn-view" onclick="editBenefit('${escapeHtml(x.id)}')">แก้ไข</button><button class="btn btn-danger" onclick="deleteBenefit('${escapeHtml(x.id)}')">ลบ</button></div></div>`).join('')||'<div class="empty">ยังไม่มีสิทธิประโยชน์</div>';if(sel){sel.innerHTML=benefitAdminRows.filter(x=>x.active).map(x=>`<option value="${escapeHtml(x.id)}">${escapeHtml(x.title)}</option>`).join('');syncBenefitUsageAmount();}const u=await api('adminBenefitUsageList',{token:adminToken});renderBenefitUsage(u.rows||[]);}catch(e){uiAlert('โหลดสิทธิประโยชน์ไม่สำเร็จ',e.message,'error');}}
function editBenefit(id){const x=benefitAdminRows.find(b=>b.id===id),f=$('#benefitForm');if(!x||!f)return;f.benefitId.value=x.id;f.title.value=x.title;f.description.value=x.description||'';f.defaultAmount.value=Number(x.defaultAmount||0);f.active.checked=!!x.active;$('#deleteBenefitBtn')?.classList.remove('hidden');}
async function deleteBenefit(id){if(!(await uiConfirm('ลบสิทธิประโยชน์','ระบบจะปิดการใช้งานสิทธิประโยชน์นี้ แต่จะเก็บประวัติการใช้สิทธิเดิมไว้ ยืนยันหรือไม่?')))return;try{await api('adminBenefitDelete',{token:adminToken,benefitId:id});$('#benefitForm')?.reset();$('#deleteBenefitBtn')?.classList.add('hidden');loadBenefitsAdmin();}catch(e){uiAlert('ลบไม่สำเร็จ',e.message,'error');}}
...
function renderBenefitUsage(rows){const host=$('#benefitUsageList');if(!host)return;host.innerHTML=rows.length?rows.map(x=>`<div class="benefit-row"><div><b>${escapeHtml(x.memberCode)} · ${escapeHtml(x.title)}</b><small>${escapeHtml(x.date)} • ${Number(x.amount||0).toLocaleString('th-TH')} บาท • ${escapeHtml(x.note||'')}</small></div></div>`).join(''):'<div class="empty">ยังไม่มีประวัติการใช้สิทธิ</div>';}
document.addEventListener('DOMContentLoaded',()=>{$('#benefitUsageSelect')?.addEventListener('change',syncBenefitUsageAmount);$('#benefitForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget;try{await api('adminBenefitSave',{token:adminToken,benefitId:f.benefitId.value,title:f.title.value,description:f.description.value,defaultAmount:f.defaultAmount.value,active:f.active.checked});f.reset();f.active.checked=true;$('#deleteBenefitBtn')?.classList.add('hidden');loadBenefitsAdmin();}catch(err){uiAlert('บันทึกไม่สำเร็จ',err.message,'error');}});$('#deleteBenefitBtn')?.addEventListener('click',()=>{const id=$('#benefitForm')?.benefitId?.value;if(id)deleteBenefit(id);});$('#benefitUsageForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget;try{await api('adminBenefitUsageSave',{token:adminToken,memberCode:f.memberCode.value,benefitId:f.benefitId.value,usedAt:f.usedAt.value,amount:f.amount.value,note:f.note.value,postExpense:f.postExpense.checked});f.reset();f.postExpense.checked=true;loadBenefitsAdmin();await loadFinanceSummary();}catch(err){uiAlert('บันทึกไม่สำเร็จ',err.message,'error');}});});
