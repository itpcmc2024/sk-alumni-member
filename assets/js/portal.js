...
async function portalLogin(){
  ...
  try{setLoading(true);const o=await api('memberPortalLogin',{memberCode,identifier});portalSession=o.session;portalData=o;renderPortal(o);$('#portalLoginBox').classList.add('hidden');$('#portalContent').classList.remove('hidden');}
  catch(e){uiAlert('เข้าสู่ข้อมูลสมาชิกไม่สำเร็จ',e.message,'error');}finally{setLoading(false);}
}
function portalLogout(){portalSession=null;portalData=null;$('#portalContent')?.classList.add('hidden');$('#portalLoginBox')?.classList.remove('hidden');}
